...
| Floor plans | Up to 30 named pages and 300 tables, with editable names/numbers, seating capacity, page, shape and percentage position/size. Circle/oval, rectangle and rounded rectangle. Numeric editing works on phones; drag editing is not yet included. |
...
| Printing | Shared persisted queue, 58/80 mm print layouts and system print dialog. The bar computer uses its installed printer driver; connectivity depends on the hardware/OS. Claim and print-attempt records prevent another device automatically printing the same ticket. Physical success is manually confirmed. Failed/uncertain jobs require an explicit reasoned COPY. |
...
- [QZ Tray printing](https://qz.io/docs/pixel) and [message signing](https://qz.io/docs/signing) were reviewed as a future desktop bridge. No QZ integration, trusted signing credentials or unattended print support is claimed in this release.
...
1. Run the complete test gate. Apply all six compliance/hospitality migrations together to a representative **staging** copy; verify schema compatibility, live memberships and RLS. Drain old clients for the coordinated release.
...
Still required for the broader requested hospitality product: unattended printer bridge with trusted signing/device setup; authenticated deployment and physical device acceptance; finished fiscal invoice/IVA/payment integration; terminal capture/refund reconciliation; cash shifts/drawer reconciliation; discounts, tips and service charge policy; modifiers/course firing and richer configurable preparation stations; drag-and-drop layout editing; accessible/mobile visual acceptance; full offline recovery if offered. These are not silently represented as implemented by the available screens.
