(function () {
  ...
  window.createCompanyManager = function(ctx) {
    ...
    function table() {
      ...
      <select aria-label="Company filter" data-company-filter>${[["all","All companies"],["activities","Activities & venues"],["accommodation","Accommodation"],["accommodation_review","Accommodation: mixed labels"],["imported","Imported companies"],["unpublished","Not on the map"],["review","Publishing needs review"]].map(([v,t])=>`<option value="${v}" ${view===v?"selected":""}>${t}</option>`).join("")}</select>${ctx.admin()?btn("history","Backups & import history"):""}${btn("refresh","Refresh")}</div>
      ...
      ${page.rows.map(r=>`<tr><td><strong>${esc(r.data.name)}</strong><div class="subtle">${esc(r.data.address)}</div>${r.accommodation_review?`<small class="subtle">Mixed label · source: ${esc(r.accommodation_review.category)}</small>`:""}</td><td>${esc(r.data.city||"—")}</td><td>${r.discovery_vertical==="accommodation"?`<span class="company-accommodation">Accommodation</span>`:esc(r.data.type||"Venue")}</td><td>${esc(r.data.catalogueVenueId?"On map":r.publish_state==="skipped"?"Needs review":r.publish_state==="working"?"Checking address":"Not published")}</td><td>${btn("details","Details",`data-id="${esc(r.id)}"`)}</td></tr>`).join("")||`<tr><td colspan="5">${loading?"Loading companies…":"No companies match this search."}</td></tr>`}</tbody></table></div>
