  ...
  window.createCompanyManager = function(ctx) {
    ...
    function dialog() {
      ...
      if(m.kind==="import") {
        ...
        body=`<h2>Publish companies to map</h2><p>Addresses are verified through the existing publishing service. Successful businesses become unclaimed venues. Ambiguous addresses are saved for review.</p><p>Up to 10 parallel checks. Progress is saved after each company. Keep this tab open, or pause and resume later.</p><p><strong>${number(m.checked)}</strong> checked in this session · ${number(m.published)} published or linked · ${number(m.skipped)} need review</p>${active?btn("pause","Pause after current checks"):btn("run-publish",m.started?"Resume publishing":"Start publishing")}`;
