(() => {
  ...
  const navSections = [
    ...
    },
    {
      label: "Operations",
      items: [
        ["inventory", "inventory"], ["pos", "sales"], ["sales", "sales"], ["marketing", "marketing"], ["sites", "sites"], ["automation", "automation"]
      ]
    },
    {
      label: "Team & insight",
  ...
  const routeToolMap = {
    ...
    employees: "employees", knowledge: "knowledge", analytics: "analytics",
    integrations: "integrations", "business-workspaces": "business-admin", settings: "dashboard"
  };

  function isPlatformWorkspace() {
    return state?.workspace?.id === "ws_akipasa";
  }

  function canUseTool(toolKey) {
    const tools = state?.workspace?.toolKeys;
  ...
  function visibleNavSections() {
    return navSections
      .map(section => ({ ...section, items: section.items.filter(([route]) => canUseRoute(route)) }))
  ...
  function seedState() {
    ...
    return {
      ...
      createdAt: isoNow(),
      workspace: {
        id: "ws_akipasa", name: "AkiPasa HQ", slug: "akipasa", timezone: "Europe/Madrid",
        currency: "EUR", ownerId: "", role: "owner", seatLimit: 100,
  ...
  let analyticsError = "";
  let analyticsUserResults = [];
  let analyticsUserSearchLoading = false;
  let analyticsUserSearchError = "";
  let analyticsUserSearchTimer = null;
  let businessWorkspaceOverview = [];
  let businessWorkspaceLoading = false;
  let businessWorkspaceError = "";
  let workspaceSwitchingId = "";
  ...
  let telegramOverviewState = { configured: false, messages: [], backups: [], refreshedAt: "" };
  let telegramLoading = false;
  let telegramLoaded = false;
  let telegramNeedsRender = false;
  let telegramFailureCount = 0;
  let telegramNextRetryAt = 0;
  ...
  async function loadCommerceData(sbClient) {
    ...
    try {
          ...
          state.products = (inventoryItems || []).map(item => {
            const cached = cachedBySku.get(String(item.sku || "").toLowerCase()) || {};
            const recommendation = recommendationByItem.get(item.id) || {};
            return {
              ...cached,
              ...
              unit: item.unit || "unit",
              warehouse: cached.warehouse || "Main",
              category: item.category || cached.category || "General",
              price: Number(item.sale_price_cents || 0) / 100,
              cost: Number(item.cost_cents || 0) / 100,
              status: item.active ? "Active" : "Archived",
              expectedDaily: Number(recommendation.expected_daily || 0),
              daysRemaining: recommendation.days_remaining == null ? null : Number(recommendation.days_remaining),
              suggestedOrder: Number(recommendation.suggested_order || 0),
              updatedAt: item.updated_at || cached.updatedAt || isoNow()
            };
          });
    ...
    try {
      ...
      try {
        ...
        if (venueRows?.length) {
          ...
          const liveCompanyAddresses = new Set(liveCompanies.map(company => normalizeProspectText(company.name) + "|" + normalizeProspectText(company.address)));
          state.companies = [
            ...liveCompanies,
            ...importedCompanies.filter(company =>
              !liveCompanyIds.has(company.id)
              && !liveCompanyIds.has(company.catalogueVenueId)
              && !liveCompanyKeys.has(normalizeProspectText(company.name) + "|" + normalizeProspectText(company.city))
              && !liveCompanyAddresses.has(normalizeProspectText(company.name) + "|" + normalizeProspectText(company.address))
            )
          ];
        }
      } catch (e) {}
  ...
  async function loadBusinessWorkspaceOverview(showToast = false) {
    if (businessWorkspaceLoading || !isPlatformWorkspace()) return;
    businessWorkspaceLoading = true;
    businessWorkspaceError = "";
    if (ui.route === "business-workspaces") render();
    try {
      const client = getSupabaseClient();
      if (!client) throw new Error("Supabase is unavailable.");
      const { data, error } = await client.rpc("crm_staff_business_overview");
      if (error) throw error;
      businessWorkspaceOverview = Array.isArray(data) ? data : [];
      if (showToast) toast("Business CRM refreshed", `${businessWorkspaceOverview.length} business account${businessWorkspaceOverview.length === 1 ? "" : "s"} loaded.`);
    } catch (error) {
      businessWorkspaceError = error.message || "The business workspace overview could not be loaded.";
      if (showToast) toast("Business CRM unavailable", businessWorkspaceError, "danger");
    } finally {
  ...
  function captureFormValues(root = document) {
  ...
  function restoreFormValues(root, values) {
    if (!values?.size) return;
    root.querySelectorAll?.("form[data-form]").forEach(form => {
      const fields = values.get(formDraftKey(form));
      if (!fields) return;
  ...
  function captureScrollPositions(root = document) {
    ...
    root.querySelectorAll?.("*").forEach(element => {
      ...
      positions.set(`${identity}:${occurrence}`, { left: element.scrollLeft, top: element.scrollTop });
    });
    return positions;
  }
  ...
  function restoreScrollPositions(root, positions) {
    ...
    root.querySelectorAll?.("*").forEach(element => {
      ...
      const occurrence = seen.get(identity) || 0;
      seen.set(identity, occurrence + 1);
      const position = positions.get(`${identity}:${occurrence}`);
      if (position) {
  ...
  function addActivity(entityType, entityId, verb, detail = "", iconName = "activity") {
    state.activities.unshift({ id: uid("ac"), workspaceId: state.workspace?.id || "ws_akipasa", entityType, entityId, actorId: state.currentUserId, verb, detail, at: isoNow(), icon: iconName });
  ...
  function setRoute(route) {
    if (!canUseRoute(route)) {
      toast("Tool not enabled", "This tool is not included in the current workspace.", "info");
      return;
    }
  ...
  async function loadTelegramOverview(renderAfter = true, force = false, notifyOnError = true) {
    if (!authUser || telegramLoading || (telegramLoaded && !force) || Date.now() < telegramNextRetryAt) return false;
    telegramLoading = true;
    let loaded = false;
    try {
      const payload = await socialGatewayRequest("/api/telegram/overview");
      telegramOverviewState = {
        configured: payload?.configured === true,
        messages: Array.isArray(payload?.messages) ? payload.messages : [],
        backups: Array.isArray(payload?.backups) ? payload.backups : [],
        refreshedAt: payload?.refreshedAt || isoNow()
      };
      telegramLoaded = true;
      ...
      telegramNextRetryAt = Date.now() + Math.min(60000, 5000 * (2 ** Math.min(telegramFailureCount - 1, 4)));
      if (force && notifyOnError) toast("Telegram unavailable", error.message || "Could not load Telegram history.", "error");
    } finally {
      telegramLoading = false;
    }
    if (loaded && renderAfter && ui.route === "telegram") render();
    return loaded;
  }

  async function refreshTelegramInBackground() {
    if (ui.route !== "telegram") return;
    const active = document.activeElement;
    const isEditing = active?.matches?.("input, textarea, select, [contenteditable='true']");
    const before = JSON.stringify({
      configured: telegramOverviewState.configured,
      messages: telegramOverviewState.messages,
      backups: telegramOverviewState.backups
    });
    const loaded = await loadTelegramOverview(false, true, false);
    if (!loaded || ui.route !== "telegram") return;
    const after = JSON.stringify({
      configured: telegramOverviewState.configured,
      messages: telegramOverviewState.messages,
      backups: telegramOverviewState.backups
    });
    telegramNeedsRender ||= before !== after;
    if (isEditing || !telegramNeedsRender) return;
    const page = document.getElementById("page-content");
    const scrollTop = page?.scrollTop || 0;
    const scrollLeft = page?.scrollLeft || 0;
    telegramNeedsRender = false;
    render();
    const nextPage = document.getElementById("page-content");
    if (nextPage) {
      nextPage.scrollTop = scrollTop;
      nextPage.scrollLeft = scrollLeft;
    }
  }
  async function loadMailboxOverview(renderAfter = true, force = false) {
    ...
    try {
      ...
      const nextMailboxOverviewState = {
      ...
      changed = JSON.stringify(mailboxOverviewState) !== JSON.stringify(nextMailboxOverviewState);
      mailboxOverviewState = nextMailboxOverviewState;
      ...
      if (renderAfter && changed) requestBackgroundRender();
    }
  }
  ...
  async function loadAnalyticsOverview(showToast = false) {
    ...
    try {
      ...
      const [analyticsResult, advertisingResult] = await Promise.all([
        client.rpc("crm_analytics_overview", { p_since: since }),
        client.rpc("crm_advertising_profile_overview", { p_since: since })
      ]);
      if (analyticsResult.error) throw analyticsResult.error;
      if (advertisingResult.error) throw advertisingResult.error;
      analyticsOverview = { ...(analyticsResult.data || {}), advertising_profiles: advertisingResult.data || {} };
  ...
  async function searchAnalyticsUsers() {
    ...
    analyticsUserSearchLoading = true;
    analyticsUserSearchError = "";
    if (ui.route === "analytics") render();
    try {
      const client = getSupabaseClient();
      ...
      const days = clamp(Number(ui.analyticsRange || 30), 7, 365);
      const since = new Date(Date.now() - days * 86400000).toISOString();
      const { data, error } = await client.rpc("crm_user_analytics_search", { p_query: query, p_since: since, p_limit: 12 });
      if (error) throw error;
      analyticsUserResults = Array.isArray(data) ? data : [];
    } catch (error) {
      console.error("Analytics user search:", error);
      analyticsUserResults = [];
      analyticsUserSearchError = error.message || "User analytics could not be loaded.";
    } finally {
      analyticsUserSearchLoading = false;
      if (ui.route === "analytics") render();
    }
  }
  ...
  function pageMeta() {
    const meta = {
      ...
      pos: [t("pos"), "Fast checkout connected to live inventory"],
  ...
  function render() {
    ...
    const formValues = captureFormValues(document);
    ...
    appRoot.innerHTML = `
      ...
      ${renderMobileTabbar()}`;
    renderPortal();
    restoreFormValues(document, formValues);
    restoreScrollPositions(document, scrollPositions);
    attachAfterRender();
    requestAnimationFrame(() => {
      const nextPage = document.getElementById("page-content");
      if (nextPage) {
  ...
  function renderSidebar() {
    ...
    return `
      <aside class="sidebar" aria-label="Primary navigation">
        ...
        <button class="sidebar-workspace-switcher" data-action="workspace-menu" aria-haspopup="menu" aria-expanded="${ui.dropdown === "workspace" ? "true" : "false"}">
          <span class="workspace-dot"></span>
          <span class="sidebar-workspace-copy"><small>Workspace</small><strong>${escapeHtml(state.workspace.name)}</strong></span>
  ...
  function renderMobileTabbar() {
    ...
    ].filter(([route]) => canUseRoute(route));
  ...
  function renderTopbar() {
    ...
    return `
      <header class="topbar">
        ...
        <button class="workspace-switcher ${workspaceSwitchingId ? "is-switching" : ""}" data-action="workspace-menu" title="Workspace switcher" aria-label="Switch workspace: ${escapeHtml(state.workspace.name)}" aria-haspopup="menu" aria-expanded="${ui.dropdown === "workspace" ? "true" : "false"}">
          <span class="workspace-dot"></span>
  ...
  function renderContextControls() {
    ...
    switch (ui.route) {
      case "crm":
        return `
          <div class="segmented">
            ${["deals", "leads", "contacts", "companies", ...(isPlatformWorkspace() ? ["social"] : []), ...(authRole === "administrator" && isPlatformWorkspace() ? ["ai-team"] : [])].map(tab => `<button class="${ui.crmTab === tab ? "active" : ""}" data-action="set-crm-tab" data-tab="${tab}">${escapeHtml(tab === "ai-team" ? t("aiTeam") : t(tab))}</button>`).join("")}
          </div>
          ${ui.crmTab === "social" ? `
      ...
      case "tasks":
        return `
          <div class="segmented">
            ...
            <button class="${ui.taskView === "projects" ? "active" : ""}" data-action="set-task-view" data-view="projects">${icon("folder")} Projects</button>
          </div>
          <span class="context-spacer"></span>
  ...
  function renderView() {
    ...
    if (!canUseRoute(ui.route)) {
      return `<div class="view"><section class="panel"><div class="panel-empty"><div><strong>Tool not enabled</strong><span>This module is not included in the current workspace. Ask a workspace owner about Business Pro tools.</span></div></div></section></div>`;
    }
          ...
          <table class="data-table">
            ...
            <tbody>
              ${state.leads.map(lead => `<tr class="clickable" data-action="view-entity" data-entity="lead" data-id="${lead.id}">
                ...
                <td><div class="row-actions"><button class="mini-btn" data-action="edit-entity" data-entity="lead" data-id="${lead.id}">${icon("edit")}</button></div></td>
              </tr>`).join("")}
            </tbody>
          </table>
        </div>
          ...
          <table class="data-table">
            ...
            <tbody>
              ${state.companies.map(company => `<tr class="clickable" data-action="view-entity" data-entity="company" data-id="${company.id}">
                ...
                <td><div class="row-actions"><button class="mini-btn" data-action="publish-company-unclaimed" data-id="${company.id}" title="${company.catalogueVenueId ? "Already published as an unclaimed venue" : "Publish as an unclaimed venue and add to map"}" ${company.catalogueVenueId ? "disabled" : ""}>${icon("arrowRight")}</button><button class="mini-btn" data-action="edit-entity" data-entity="company" data-id="${company.id}">${icon("edit")}</button><button class="mini-btn danger" data-action="delete-entity" data-entity="company" data-id="${company.id}" title="Delete venue">${icon("trash")}</button></div></td>
              </tr>`).join("")}
            </tbody>
          </table>
        </div>
  ...
  function renderProjectTaskOverview(projectId, tasks) {
    const expanded = ui.expandedProjectIds.includes(projectId);
    const completed = tasks.filter(task => task.status === "done").length;
    const toggleLabel = tasks.length
      ? (expanded ? "Hide tasks" : "View " + tasks.length + " task" + (tasks.length === 1 ? "" : "s"))
      : "No linked tasks";
    const rows = tasks.map(task => {
      const isDone = task.status === "done";
      return '<button class="project-task-row" data-action="view-entity" data-entity="task" data-id="' + escapeHtml(task.id) + '">' +
        '<span class="project-task-check ' + (isDone ? "complete" : "") + '">' + (isDone ? icon("check") : "") + '</span>' +
        '<span class="project-task-copy"><strong>' + escapeHtml(task.title) + '</strong><small>' +
        escapeHtml(capitalize(task.status)) + ' · ' + escapeHtml(formatDate(task.dueDate)) +
        '</small></span><span class="status-pill ' + (isDone ? "success" : task.status === "progress" ? "info" : "") + '">' +
        (isDone ? "Complete" : "Open") + '</span></button>';
    }).join("");
    return '<div class="project-task-overview">' +
      '<button class="project-task-toggle" data-action="toggle-project-tasks" data-id="' + escapeHtml(projectId) +
      '" aria-expanded="' + expanded + '" ' + (tasks.length ? "" : "disabled") + '>' +
      '<span>' + icon("tasks") + ' Task overview</span><strong>' + completed + '/' + tasks.length + '</strong><em>' +
      escapeHtml(toggleLabel) + '</em></button>' +
      (expanded && tasks.length ? '<div class="project-task-list">' + rows + '</div>' : "") + '</div>';
  }
  function renderTasks() {
    ...
    if (ui.taskView === "projects") {
      const orderedProjects = [...(state.projects || [])].sort((a, b) => {
        const rank = { Active: 0, Planning: 1, "On hold": 2, Completed: 3 };
        return (rank[a.status] ?? 9) - (rank[b.status] ?? 9) || String(a.name).localeCompare(String(b.name));
      });
      return `<section class="panel projects-panel">
        <div class="panel-header"><div><h2>Projects</h2><p>Plan work, review progress and keep every linked task together.</p></div><button class="action-btn primary" data-action="open-form" data-entity="project">${icon("plus")} New project</button></div>
        ${orderedProjects.length ? `<div class="project-catalogue">${orderedProjects.map(project => {
          const tasks = (state.tasks || []).filter(task => task.projectId === project.id);
          const done = tasks.filter(task => task.status === "done").length;
          const progress = clamp(tasks.length ? Math.round(done / tasks.length * 100) : Number(project.progress || 0), 0, 100);
          return `<article class="project-card" data-action="toggle-project-card" data-id="${project.id}" tabindex="0">
            <div class="project-card-top"><div class="project-mark">${icon("folder")}</div><span class="status-pill ${project.status === "Completed" ? "success" : project.status === "On hold" ? "warning" : project.status === "Active" ? "info" : ""}">${escapeHtml(project.status || "Planning")}</span><div class="row-actions"><button class="mini-btn" data-action="edit-entity" data-entity="project" data-id="${project.id}" title="Edit project">${icon("edit")}</button><button class="mini-btn" data-action="delete-entity" data-entity="project" data-id="${project.id}" title="Delete project">${icon("trash")}</button></div></div>
            <h3>${escapeHtml(project.name)}</h3>
            <div class="project-meta"><span>${escapeHtml(employeeName(project.ownerId))}</span><span>${tasks.length} tasks</span><span>${done} complete</span></div>
            <div class="project-progress"><div><span>Progress</span><strong>${progress}%</strong></div><div class="progress-track"><i style="width:${progress}%"></i></div></div>
            ${renderProjectTaskOverview(project.id, tasks)}
            <div class="project-foot"><span class="${project.status !== "Completed" && isOverdue(project.dueDate) ? "text-danger" : ""}">${icon("calendar")} ${escapeHtml(formatDate(project.dueDate))}</span><strong>${escapeHtml(formatMoney(project.budget || 0))}</strong></div>
          </article>`;
        }).join("")}</div>` : `<div class="empty-state project-empty">${icon("folder")}<h3>No projects yet</h3><p>Create a project to group tasks, deadlines and ownership.</p><button class="action-btn primary" data-action="open-form" data-entity="project">${icon("plus")} Create your first project</button></div>`}
      </section>`;
    }
    if (ui.taskView === "list") {
      return `
  ...
  function renderInventory() {
    ...
    return `<div class="page-grid">
      ...
      <section class="panel table-panel">
        ...
        <div class="table-scroll">
          <table class="data-table">
            <thead><tr><th>Item</th><th>SKU</th><th>Category</th><th>Sell / cost</th><th>On hand</th><th>Runway</th><th>Status</th><th></th></tr></thead>
            <tbody>${visibleProducts.map(product => `<tr class="clickable" data-action="view-entity" data-entity="product" data-id="${product.id}">
              ...
              <td><strong>${escapeHtml(formatMoney(product.price))}</strong><br><span class="subtle">${escapeHtml(formatMoney(product.cost))} cost</span></td>
              <td class="${Number(product.stock) <= Number(product.reorderAt) && product.warehouse !== "Digital" ? "text-danger" : ""}"><strong>${Number(product.stock).toLocaleString()}</strong> ${escapeHtml(product.unit || "")}</td>
              <td>${product.daysRemaining == null ? "—" : `${Number(product.daysRemaining).toLocaleString()} days`}</td>
            ...
            </tr>`).join("") || `<tr><td colspan="8"><div class="panel-empty compact"><div><strong>No matching items</strong><span>Change the filter or add an inventory item.</span></div></div></td></tr>`}</tbody>
          </table>
        </div>
      </section>
      <section class="panel table-panel">
        <div class="panel-header"><div><h2>Stock movement ledger</h2><p>Purchases, sales, waste, returns and manual counts for this workspace only.</p></div></div>
        <div class="table-scroll"><table class="data-table"><thead><tr><th>Item</th><th>Movement</th><th>Quantity</th><th>Note</th><th>When</th></tr></thead><tbody>${inventoryMovements.slice(0, 30).map(movement => `<tr><td><strong>${escapeHtml(movementName(movement))}</strong></td><td><span class="status-pill ${Number(movement.quantity_delta) < 0 ? "warning" : "success"}">${escapeHtml(capitalize(movement.movement_type))}</span></td><td class="${Number(movement.quantity_delta) < 0 ? "text-danger" : ""}">${Number(movement.quantity_delta) > 0 ? "+" : ""}${Number(movement.quantity_delta).toLocaleString()}</td><td>${escapeHtml(movement.notes || "—")}</td><td>${escapeHtml(relativeTime(movement.occurred_at))}</td></tr>`).join("") || `<tr><td colspan="5"><div class="panel-empty compact"><div><strong>No stock movements yet</strong><span>Add an item or receive stock to start the ledger.</span></div></div></td></tr>`}</tbody></table></div>
      </section>
    </div>`;
  }
  ...
  const analyticsTuningFields = [
    ["category_affinity", "Category affinity", "How strongly learned category preferences influence ranking."],
    ["venue_affinity", "Venue affinity", "How strongly a visitor's past venue interactions influence ranking."],
    ["price_affinity", "Price affinity", "How strongly price preferences influence ranking."],
    ["distance_relevance", "Distance relevance", "How strongly distance from the visitor influences ranking."],
    ["temporal_relevance", "Timing relevance", "How strongly date and time relevance influence ranking."],
    ["quality", "Quality", "How strongly catalogue quality signals influence ranking."],
    ["freshness", "Freshness", "How strongly newer catalogue items are preferred."],
    ["session_intent", "Session intent", "How strongly the current browsing intent influences ranking."],
    ["repetition_penalty", "Repetition penalty", "How strongly repeated items are pushed down."],
    ["negative_affinity", "Negative feedback", "How strongly skips, exits and dislikes suppress items."]
  ];

  function analyticsTuningFromRanking(ranking = {}) {
    const weights = ranking.weights || {};
    const defaults = { category_affinity: .24, venue_affinity: .18, price_affinity: .08, distance_relevance: .15, temporal_relevance: .13, quality: .10, freshness: .06, session_intent: .16, repetition_penalty: .12, negative_affinity: .28 };
    return { ...Object.fromEntries(Object.keys(defaults).map(key => [key, Number.isFinite(Number(weights[key])) ? Number(weights[key]) : defaults[key]])), exploration_ratio: Number.isFinite(Number(ranking.exploration_ratio)) ? Number(ranking.exploration_ratio) : .15, sponsored_multiplier: Number.isFinite(Number(ranking.sponsored_multiplier)) ? Number(ranking.sponsored_multiplier) : 1.12, sponsored_minimum_relevance: Number.isFinite(Number(ranking.sponsored_minimum_relevance)) ? Number(ranking.sponsored_minimum_relevance) : .25 };
  }

  function currentAnalyticsTuning() {
    if (!ui.analyticsTuningDraft) ui.analyticsTuningDraft = analyticsTuningFromRanking(analyticsOverview?.active_ranking);
    return ui.analyticsTuningDraft;
  }

  function renderAnalyticsTuning() {
    const tuning = currentAnalyticsTuning();
    const controls = analyticsTuningFields.map(([key, label, help]) => `<label class="analytics-tuning-control"><span><strong>${escapeHtml(label)}</strong><b data-analytics-tuning-value="${key}">${Math.round(Number(tuning[key] || 0) * 100)}%</b></span><small>${escapeHtml(help)}</small><input type="range" min="0" max="100" step="1" value="${Math.round(Number(tuning[key] || 0) * 100)}" data-analytics-tuning="${key}"></label>`).join("");
    return `<section class="panel analytics-tuning-panel"><div class="panel-header"><div><h2>Ranker experiment controls</h2><p>Stage safe weight changes here, compare the effect, then explicitly apply them to the live recommendation ranker.</p></div><span class="status-pill info">Admin only</span></div><div class="analytics-tuning-grid">${controls}<label class="analytics-tuning-control"><span><strong>Exploration</strong><b data-analytics-tuning-value="exploration_ratio">${Math.round(Number(tuning.exploration_ratio || 0) * 100)}%</b></span><small>How often the system explores less familiar catalogue items.</small><input type="range" min="0" max="50" step="1" value="${Math.round(Number(tuning.exploration_ratio || 0) * 100)}" data-analytics-tuning="exploration_ratio"></label><label class="analytics-tuning-control"><span><strong>Sponsored multiplier</strong><b data-analytics-tuning-value="sponsored_multiplier">${Number(tuning.sponsored_multiplier || 1).toFixed(2)}×</b></span><small>Maximum boost applied to relevant sponsored placements.</small><input type="range" min="100" max="200" step="1" value="${Math.round(Number(tuning.sponsored_multiplier || 1.12) * 100)}" data-analytics-tuning="sponsored_multiplier"></label><label class="analytics-tuning-control"><span><strong>Sponsored relevance floor</strong><b data-analytics-tuning-value="sponsored_minimum_relevance">${Math.round(Number(tuning.sponsored_minimum_relevance || 0) * 100)}%</b></span><small>Minimum relevance required before sponsored placement is eligible.</small><input type="range" min="0" max="100" step="1" value="${Math.round(Number(tuning.sponsored_minimum_relevance || 0.25) * 100)}" data-analytics-tuning="sponsored_minimum_relevance"></label></div><div class="analytics-tuning-actions"><button class="action-btn" data-action="reset-analytics-tuning">Reset draft</button><button class="action-btn primary" data-action="apply-analytics-tuning">Apply to live ranker</button></div><p class="analytics-tuning-warning">Applying changes creates a new ranker version for all public recommendation surfaces. Start with small adjustments and monitor this dashboard.</p></section>`;
  }
  ...
  function renderAnalytics() {
    ...
    const advertising = overview.advertising_profiles || {};
    const adCoverage = advertising.coverage || {};
    const adQuality = advertising.quality || {};
    const adSegments = advertising.top_segments || [];
    const adSources = advertising.source_mix || [];
    ...
    const renderUserSearch = mode => {
      const query = String(ui.analyticsUserSearch || "").trim();
      const results = analyticsUserResults.map(user => {
        const affinities = Array.isArray(user.top_affinities) ? user.top_affinities : [];
        if (mode === "advertising") {
          const eligible = user.advertising_status === "eligible";
          const readiness = Number(user.signal_count || 0) >= 5 ? "Ready" : Number(user.signal_count || 0) > 0 ? "Learning" : "No learned profile";
          return `<div><span>${icon("user")}</span><strong>${escapeHtml(user.display_name || "AkiPasa user")}<small>${escapeHtml(user.email || "No email")}</small></strong><small>${escapeHtml(capitalize(user.advertising_status || "awaiting_consent"))} · marketing ${user.marketing_enabled ? "on" : "off"} · personalisation ${user.personalisation_enabled ? "on" : "off"}<br>${escapeHtml(readiness)} · ${number(user.signal_count)} signals · ${Math.round(Number(user.average_confidence || 0) * 100)}% confidence</small><b class="status-pill ${eligible ? "success" : ""}">${eligible ? "Eligible" : "Not eligible"}</b></div>`;
        }
        return `<div><span>${icon("user")}</span><strong>${escapeHtml(user.display_name || "AkiPasa user")}<small>${escapeHtml(user.email || "No email")}</small></strong><small>${number(user.event_count)} events · ${number(user.recommendation_count)} recommendations · last active ${escapeHtml(user.last_active_at ? relativeTime(user.last_active_at) : "never")}<br>${affinities.length ? affinities.map(item => `${escapeHtml(capitalize(item.key))} (${Math.round(Number(item.confidence || 0) * 100)}%)`).join(" · ") : "No learned affinities"}</small><b>${number(user.signal_count)}</b></div>`;
      }).join("");
      return `<div class="panel-header"><div><h2>${mode === "advertising" ? "Find an advertising profile" : "Find user analytics"}</h2><p>Search a registered user by name or email. Results are administrator-only summaries.</p></div><input class="toolbar-input" style="width:min(360px,100%);height:36px" type="search" data-analytics-user-search value="${escapeHtml(ui.analyticsUserSearch || "")}" placeholder="Search name or email…" autocomplete="off"></div><div class="panel-body analytics-ranked-list">${analyticsUserSearchLoading ? `<div class="analytics-inline-empty">Searching…</div>` : analyticsUserSearchError ? `<div class="analytics-inline-empty">${escapeHtml(analyticsUserSearchError)}</div>` : query.length < 2 ? `<div class="analytics-inline-empty">Enter at least 2 characters to inspect a user summary.</div>` : results || `<div class="analytics-inline-empty">No registered users matched that search.</div>`}</div>`;
    };
    ...
    return `<div class="analytics-dashboard">
      ...
      <section class="panel">${renderUserSearch("analytics")}</section>
      ...
      <section class="panel analytics-business-panel">
        <div class="panel-header"><div><h2>Advertising profile overview</h2><p>Consent-aware first-party audience readiness. Profiles remain empty until both personalisation and marketing consent are enabled.</p></div><span class="status-pill success">Aggregate only</span></div>
        <div class="analytics-business-grid">
          <div><span>Registered coverage</span><strong>${number(adCoverage.profile_shells)} / ${number(adCoverage.registered_users)}</strong><small>${percent(adCoverage.profile_shells, adCoverage.registered_users)}% have a lifecycle record</small></div>
          <div><span>Eligible profiles</span><strong>${number(adCoverage.eligible)}</strong><small>${number(adCoverage.awaiting_consent)} awaiting consent · ${number(adCoverage.opted_out)} opted out</small></div>
          <div><span>Audience ready</span><strong>${number(adCoverage.ready)}</strong><small>${number(adCoverage.learning)} still learning · ${number(adCoverage.fresh_30d)} fresh in 30 days</small></div>
          <div><span>Signal quality</span><strong>${number(adQuality.total_signals)}</strong><small>${Math.round(Number(adQuality.average_confidence || 0) * 100)}% average confidence · ${number(adQuality.profiles_with_signals)} profiled users</small></div>
        </div>
        ${renderUserSearch("advertising")}
        <div class="analytics-detail-grid analytics-lower-grid">
          <section><div class="panel-header"><div><h2>Top ad segments</h2><p>Non-sensitive positive affinities among eligible users</p></div></div><div class="panel-body analytics-ranked-list">${adSegments.map((item, index) => `<div><span>${index + 1}</span><strong>${escapeHtml(capitalize(item.key))}</strong><small>${escapeHtml(capitalize(item.dimension))} · ${Math.round(Number(item.confidence || 0) * 100)}% confidence</small><b>${number(item.profiles)}</b></div>`).join("") || `<div class="analytics-inline-empty">Segments appear after consenting users generate enough preference signals.</div>`}</div></section>
          <section><div class="panel-header"><div><h2>Profile signal sources</h2><p>Consented behavioural inputs in this reporting window</p></div></div><div class="panel-body analytics-ranked-list">${adSources.map((item, index) => `<div><span>${index + 1}</span><strong>${escapeHtml(capitalize(item.source))}</strong><small>${number(item.profiles)} profiles</small><b>${number(item.signals)}</b></div>`).join("") || `<div class="analytics-inline-empty">No consented advertising-profile signals in this period.</div>`}</div></section>
        </div>
      </section>
  ...
  function renderTelegram() {
    ...
    const backups = telegramOverviewState.backups.map(item => ({
      id: item.id,
      title: `24h Telegram Backup (${String(item.period_end || item.created_at || "").slice(0, 10)})`,
      messagesCount: Number(item.message_count || 0),
      createdAt: item.created_at,
      status: "Stored"
    }));
    const messages = telegramOverviewState.messages.map(item => ({
      id: item.id,
      from: item.username ? `@${item.username}` : item.direction === "outbound" ? "@AkiPasaHQBot" : `Telegram ${item.user_id || "user"}`,
      user: item.display_name || (item.direction === "outbound" ? "AkiPasa Bot" : "Telegram user"),
      text: item.direction === "inbound" ? item.body : "",
      response: item.direction === "outbound" ? item.body : "",
      command: item.command || "chat",
      at: item.created_at,
      direction: item.direction
    }));

    const query = (ui.telegramSearch || "").trim().toLowerCase();
    const filteredMessages = messages.filter(m => !query || `${m.from} ${m.user} ${m.text} ${m.response} ${m.command}`.toLowerCase().includes(query));
    ...
    if (tab === "chats") {
      tabContent = `
        <section class="panel table-panel">
          ...
          <div class="table-scroll">
          <form data-form="telegram-chat" class="telegram-chat-composer">
            <textarea name="message" maxlength="3500" rows="3" required placeholder="Write to the Telegram group as ${escapeHtml(currentUser().name)}..."></textarea>
            <div>
              <span>Sent by @AkiPasaHQBot and signed <strong>-${escapeHtml(currentUser().name)}</strong></span>
              <button class="action-btn primary" type="submit">${icon("send")} Send to group</button>
            </div>
          </form>
            <table class="data-table">
      ...
      tabContent = `
        <section class="panel table-panel">
          <div class="panel-header">
            <div><h2>Automated 24h Chat Backups</h2><p>${backups.length} archived snapshots · scheduled every 24 hours</p></div>
            <div class="panel-actions">
              <button class="action-btn primary" data-action="trigger-telegram-backup">${icon("refresh")} 24h Backup Now</button>
            </div>
          </div>
        ...
        <div class="page-grid grid-3">
          ${commandsList.map(item => `
            <article class="panel" style="padding:16px">
              ...
              <button class="mini-btn primary" data-action="prefill-telegram-command" data-cmd="${escapeHtml(item.cmd.split(" ")[0])}" style="width:100%;justify-content:center">
                ${icon("send")} Use in chat
              </button>
            </article>
          `).join("")}
        </div>
    ...
    return `
      <div class="page-grid">
        <div class="page-grid grid-4">
          ...
          ${renderMetric("24h Chat backups", `${backups.length} Saved`, "Auto-backup every 24 hours", "download", "#49d7a0")}
          ${renderMetric("Messages logged", String(messages.length), "Real-time group & bot sync", "inbox", "#7c8cff")}
  ...
  function renderBusinessWorkspaces() {
    if (!isPlatformWorkspace()) return `<section class="panel"><div class="panel-empty"><div><strong>Platform workspace required</strong><span>Business CRM administration is available only inside AkiPasa HQ.</span></div></div></section>`;
    ...
    const toolLabels = {
      ...
      knowledge: "Knowledge", analytics: "Analytics", integrations: "Integrations"
    };
    const query = ui.businessSearch.trim().toLowerCase();
    const matchingBusinesses = businessWorkspaceOverview.filter(business => {
      const workspaces = Array.isArray(business.workspaces) ? business.workspaces : [];
      const matchesQuery = !query || `${business.display_name || ""} ${business.profile_id || ""} ${workspaces.map(workspace => `${workspace.name || ""} ${workspace.id || ""}`).join(" ")}`.toLowerCase().includes(query);
      const matchesTier = ui.businessTierFilter === "all" || business.business_tier === ui.businessTierFilter;
      return matchesQuery && matchesTier;
    });
    const pageSize = 20;
    const pageCount = Math.max(1, Math.ceil(matchingBusinesses.length / pageSize));
    const currentPage = Math.min(ui.businessPage, pageCount);
    const pageBusinesses = matchingBusinesses.slice((currentPage - 1) * pageSize, currentPage * pageSize);
    const workspaceCount = businessWorkspaceOverview.reduce((sum, item) => sum + (Array.isArray(item.workspaces) ? item.workspaces.length : 0), 0);
    return `<div class="page-grid">
      <section class="panel business-admin-overview">
        <div class="business-admin-summary">
          <div><strong>${formatCount(businessWorkspaceOverview.length)}</strong><span>accounts</span></div>
          <div><strong>${formatCount(businessWorkspaceOverview.filter(item => item.business_tier === "business_pro").length)}</strong><span>Business Pro</span></div>
          <div><strong>${formatCount(workspaceCount)}</strong><span>workspaces</span></div>
          <button class="action-btn" data-action="refresh-business-workspaces">${icon("refresh")} Refresh</button>
        </div>
        <div class="business-admin-toolbar">
          <label class="business-admin-search">${icon("search")}<input type="search" data-business-search autocomplete="off" placeholder="Search account, company or workspace ID" value="${escapeHtml(ui.businessSearch)}"></label>
          <select data-change="business-tier" aria-label="Filter by business tier">
            <option value="all" ${ui.businessTierFilter === "all" ? "selected" : ""}>All tiers</option>
            <option value="business_pro" ${ui.businessTierFilter === "business_pro" ? "selected" : ""}>Business Pro</option>
            <option value="business" ${ui.businessTierFilter === "business" ? "selected" : ""}>Business</option>
            <option value="none" ${ui.businessTierFilter === "none" ? "selected" : ""}>Unassigned</option>
          </select>
          <span class="business-result-count">${formatCount(matchingBusinesses.length)} result${matchingBusinesses.length === 1 ? "" : "s"}</span>
        </div>
      </section>
      <div class="business-admin-list">
      ${pageBusinesses.map(business => {
        const workspaces = Array.isArray(business.workspaces) ? business.workspaces : [];
        const expanded = ui.expandedBusinessIds.includes(business.profile_id);
        const enabledTools = workspaces.reduce((sum, workspace) => sum + Object.values(workspace.tools || {}).filter(Boolean).length, 0);
        const totalTools = workspaces.reduce((sum, workspace) => sum + Object.keys(workspace.tools || {}).length, 0);
        return `<section class="panel business-workspace-admin-card ${expanded ? "expanded" : ""}">
          <button class="business-account-row" data-action="toggle-business-account" data-id="${escapeHtml(business.profile_id)}" aria-expanded="${expanded ? "true" : "false"}">
            <span class="business-account-primary"><strong>${escapeHtml(business.display_name || "Business account")}</strong><small>${formatCount(business.venue_count)} venue${Number(business.venue_count) === 1 ? "" : "s"} · ${formatCount(workspaces.length)} workspace${workspaces.length === 1 ? "" : "s"}</small></span>
            <span class="status-pill ${business.business_tier === "business_pro" ? "success" : ""}">${escapeHtml(business.business_tier === "business_pro" ? "Business Pro" : business.business_tier === "business" ? "Business" : "Unassigned")}</span>
            <span class="business-tool-count">${formatCount(enabledTools)}/${formatCount(totalTools)} modules on</span>
            <span class="business-account-chevron">${icon("chevron")}</span>
          </button>
          ${expanded ? `<div class="panel-body page-grid">
            ${workspaces.length ? workspaces.map(workspace => `<article class="business-workspace-control">
              <header><div><strong>${escapeHtml(workspace.name)}</strong><span>${formatCount(workspace.seat_count)} / ${formatCount(workspace.seat_limit)} seats · ${escapeHtml(workspace.status)}</span></div><code>${escapeHtml(workspace.id)}</code></header>
              <div class="business-tool-grid">${Object.entries(workspace.tools || {}).map(([tool, enabled]) => `<button class="business-tool-toggle ${enabled ? "enabled" : ""}" data-action="toggle-business-tool" data-workspace="${escapeHtml(workspace.id)}" data-tool="${escapeHtml(tool)}" data-active="${enabled ? "true" : "false"}" aria-pressed="${enabled ? "true" : "false"}"><span>${escapeHtml(toolLabels[tool] || capitalize(tool))}</span><i>${enabled ? "On" : "Off"}</i></button>`).join("")}</div>
            </article>`).join("") : `<div class="panel-empty"><div><strong>No CRM workspace</strong><span>This account has standard Business access, or its Business Pro payment has not been activated yet.</span></div></div>`}
          </div>` : ""}
        </section>`;
      }).join("") || `<section class="panel"><div class="panel-empty"><div><strong>No matching business accounts</strong><span>Try a company name, workspace ID, or a different tier.</span></div></div></section>`}
      </div>
      ${pageCount > 1 ? `<nav class="business-pagination" aria-label="Business account pages"><button class="action-btn" data-action="business-page" data-page="${currentPage - 1}" ${currentPage <= 1 ? "disabled" : ""}>Previous</button><span>Page ${currentPage} of ${pageCount}</span><button class="action-btn" data-action="business-page" data-page="${currentPage + 1}" ${currentPage >= pageCount ? "disabled" : ""}>Next</button></nav>` : ""}
    </div>`;
  }
    ...
    const configs = {
      ...
      company: {
        label: "Company", icon: "building", fields: [
          ...
          { name: "city", label: "City" }, { name: "address", label: "Full address", full: true },
      ...
      employee: {
        label: "Team member", icon: "employees", fields: [
          ...
          { name: "role", label: "Role", required: true, type: "select", options: isPlatformWorkspace() ? ["Consumer", "Organiser", "Staff", "Admin"] : ["Admin", "Manager", "Staff", "Viewer"], value: "Staff" },
  ...
  function renderPortal() {
    const scrollPositions = captureScrollPositions(portal);
    const formValues = captureFormValues(portal);
    const parts = [];
    ...
    restoreFormValues(portal, formValues);
    restoreScrollPositions(portal, scrollPositions);
  ...
  function detailPairs(type, entity) {
    const map = {
      ...
      company: [["Type", entity.type || "—"], ["City", entity.city || "—"], ["Full address", entity.address || "—", true], ["Status", entity.status || "—"], ["Owner", employeeName(entity.ownerId)], ["Email", entity.email || "—"], ["Phone", entity.phone || "—"], ["Website", entity.website || "—"], ["Employees", entity.employees || 0]],
  ...
  function renderEntityFormModal() {
    ...
    return `<div class="modal-backdrop" data-action="close-modal"></div><section class="modal ${["automation", "article"].includes(type) ? "wide" : ""}" role="dialog" aria-modal="true">
      ...
      <form data-form="entity" data-entity="${type}" data-id="${id || ""}" style="display:contents">
        <div class="modal-body">
          ${type === "employee" && !existing ? (isPlatformWorkspace() ? renderUserPickerSection() : `<div class="notice-card"><strong>Add an existing AkiPasa account</strong><span>Enter the exact email used to sign in. Business Pro includes the owner plus three team seats.</span></div>`) : ""}
  ...
  function renderDropdown() {
    ...
    if (ui.dropdown === "workspace") {
      return `<div class="dropdown workspace-dropdown" role="menu" style="left:${state.settings.sidebarCollapsed ? "87px" : "247px"};top:58px">
        <div class="dropdown-head"><strong>Choose workspace</strong></div>
        ${availableWorkspaces.map(workspace => `<div class="dropdown-item ${workspace.id === state.workspace.id ? "unseen" : ""}" data-action="switch-workspace" data-id="${escapeHtml(workspace.id)}"><div class="activity-icon">${icon("building")}</div><div class="dropdown-copy"><strong>${escapeHtml(workspace.name)}</strong><br>${escapeHtml(workspace.id === "ws_akipasa" ? "AkiPasa platform workspace" : `Business Pro · ${workspace.role}`)}</div></div>`).join("")}
  ...
  function commandDefinitions() {
    return [
      ...visibleNavSections().flatMap(section => section.items.map(([route, iconName]) => ({ group: "Navigate", label: t(route), icon: iconName, action: "navigate", route }))),
  ...
  async function handleAction(target, event) {
    ...
    switch (action) {
      ...
      case "set-crm-tab":
        ...
        if (!isPlatformWorkspace() && ["social", "ai-team"].includes(ui.crmTab)) {
          ui.crmTab = "deals";
          toast("Platform tool", "This CRM workspace does not include AkiPasa platform operations.", "info");
        }
        if (ui.crmTab === "social") {
      ...
      case "refresh-analytics":
        ...
        break;
      case "reset-analytics-tuning":
        ui.analyticsTuningDraft = null;
        render();
        break;
      case "apply-analytics-tuning": {
        if (authRole !== "administrator") break;
        try {
          await socialGatewayRequest("/api/analytics/ranking-config", { method: "PUT", body: currentAnalyticsTuning() });
          ui.analyticsTuningDraft = null;
          analyticsOverview = null;
          await loadAnalyticsOverview(false);
          toast("Ranker updated", "The new configuration is live across recommendation surfaces.", "success");
        } catch (error) {
          toast("Ranker update failed", error.message || "The configuration could not be applied.", "danger");
        }
        break;
      }
      case "set-ai-section":
      ...
      case "set-task-view":
        ...
        break;
      case "toggle-project-tasks": {
        const projectId = target.dataset.id;
        ui.expandedProjectIds = ui.expandedProjectIds.includes(projectId)
          ? ui.expandedProjectIds.filter(id => id !== projectId)
          : [...ui.expandedProjectIds, projectId];
        render();
        break;
      }
      case "toggle-project-card": {
        const projectId = target.dataset.id;
        ui.expandedProjectIds = ui.expandedProjectIds.includes(projectId)
          ? ui.expandedProjectIds.filter(id => id !== projectId)
          : [...ui.expandedProjectIds, projectId];
      ...
      case "switch-workspace":
        await switchWorkspace(target.dataset.id);
        break;
      case "refresh-business-workspaces":
        await loadBusinessWorkspaceOverview(true);
        break;
      case "toggle-business-account": {
        const id = target.dataset.id;
        ui.expandedBusinessIds = ui.expandedBusinessIds.includes(id) ? ui.expandedBusinessIds.filter(item => item !== id) : [...ui.expandedBusinessIds, id];
        render();
        break;
      }
      case "business-page":
        ui.businessPage = Math.max(1, Number(target.dataset.page || 1));
        render();
        document.getElementById("page-content")?.scrollTo({ top: 0, behavior: state.settings.reducedMotion ? "auto" : "smooth" });
        break;
      case "toggle-business-tool": {
        const client = getSupabaseClient();
        const workspaceId = target.dataset.workspace;
        const toolKey = target.dataset.tool;
        const nextActive = target.dataset.active !== "true";
        if (!client || !workspaceId || !toolKey) break;
        target.disabled = true;
        const { error } = await client.rpc("crm_staff_set_workspace_tool", {
          p_workspace: workspaceId,
          p_tool: toolKey,
          p_active: nextActive,
          p_reason: `Changed ${toolKey} from the AkiHQ Business CRM overview`
        });
        if (error) toast("Feature not changed", error.message || "The entitlement update failed.", "danger");
        else {
          const workspace = availableWorkspaces.find(item => item.id === workspaceId);
          if (workspace) {
            workspace.toolKeys = nextActive
              ? [...new Set([...(workspace.toolKeys || []), toolKey])]
              : (workspace.toolKeys || []).filter(key => key !== toolKey);
            if (state.workspace.id === workspaceId) state.workspace.toolKeys = [...workspace.toolKeys];
          }
          toast(nextActive ? "Feature enabled" : "Feature disabled", `${capitalize(toolKey)} · ${workspaceId}`, "success");
          render();
        }
        await loadBusinessWorkspaceOverview(false);
        break;
      }
      ...
      case "convert-lead":
        ...
        break;
      case "publish-company-unclaimed":
        event.preventDefault(); event.stopPropagation();
      ...
      case "pos-decrement": {
        const id = target.dataset.id;
        const next = Number(ui.posCart[id] || 0) - 1;
        if (next > 0) ui.posCart[id] = next;
        else delete ui.posCart[id];
        render();
        break;
      }
      case "pos-clear":
        ...
        break;
      case "pos-tender":
        ui.posTender = target.dataset.value || "card";
        render();
        break;
      case "pos-toggle-cart":
        ui.posCartOpen = !ui.posCartOpen;
        render();
        break;
      case "pos-checkout":
      ...
      case "trigger-telegram-backup": {
        try {
          const payload = await socialGatewayRequest("/api/telegram/backups", { method: "POST", body: {} });
          await loadTelegramOverview(false, true);
          render();
          toast("24h Telegram backup saved", `${payload?.backup?.message_count || 0} messages archived.`, "success");
        } catch (error) {
          toast("Backup failed", error.message || "The Telegram backup could not be created.", "danger");
        }
        break;
      }
      case "export-telegram-logs": {
        const blob = new Blob([JSON.stringify(telegramOverviewState.messages, null, 2)], { type: "application/json" });
        const url = URL.createObjectURL(blob); const a = document.createElement("a");
        a.href = url; a.download = `telegram-chat-log-${isoNow().slice(0, 10)}.json`; a.click(); URL.revokeObjectURL(url);
        break;
      }
      case "download-telegram-backup": {
        try {
          const payload = await socialGatewayRequest(`/api/telegram/backups/${encodeURIComponent(target.dataset.id || "")}`);
          const blob = new Blob([JSON.stringify(payload.backup, null, 2)], { type: "application/json" });
          const url = URL.createObjectURL(blob); const a = document.createElement("a");
          a.href = url; a.download = `telegram-backup-${target.dataset.id}.json`; a.click(); URL.revokeObjectURL(url);
        } catch (error) {
          toast("Export failed", error.message || "The backup could not be downloaded.", "danger");
        }
        break;
      }
      case "prefill-telegram-command": {
        ui.telegramTab = "chats"; render();
        const composer = document.querySelector('[data-form="telegram-chat"] textarea[name="message"]');
        if (composer) { composer.value = target.dataset.cmd || ""; composer.focus(); }
        break;
      }
      case "cloud-push":
        cloudPush();
  ...
  function handleChange(target) {
    ...
    } else if (change === "business-tier") {
      ui.businessTierFilter = target.value || "all";
      ui.businessPage = 1;
      render();
  ...
  function handleInput(target) {
    ...
    if (target.matches("[data-pos-search]")) {
      ui.posSearch = target.value;
      const position = target.selectionStart;
      render();
      const input = $("[data-pos-search]");
      if (input) { input.focus(); input.setSelectionRange(position, position); }
      return;
    }
    if (target.matches("[data-analytics-tuning]")) {
      const key = target.dataset.analyticsTuning;
      const value = Number(target.value || 0) / 100;
      currentAnalyticsTuning()[key] = value;
      const display = key === "sponsored_multiplier" ? `${value.toFixed(2)}×` : `${Math.round(value * 100)}%`;
      document.querySelectorAll(`[data-analytics-tuning-value="${key}"]`).forEach(node => { node.textContent = display; });
      return;
    }
    if (target.matches("[data-business-search]")) {
      ui.businessSearch = target.value;
      ui.businessPage = 1;
      const position = target.selectionStart;
      render();
      const input = $("[data-business-search]");
      if (input) { input.focus(); input.setSelectionRange(position, position); }
      return;
    }
    if (target.matches("[data-analytics-user-search]")) {
      ui.analyticsUserSearch = target.value;
      $$('[data-analytics-user-search]').forEach(input => { if (input !== target) input.value = target.value; });
      if (analyticsUserSearchTimer) clearTimeout(analyticsUserSearchTimer);
      analyticsUserSearchTimer = setTimeout(searchAnalyticsUsers, 350);
      return;
    }
  ...
  async function saveEntityFromForm(form) {
    ...
    if (type === "product") {
      ...
      if (itemId) {
        const { data: updated, error } = await client.from("crm_inventory_items").update({
          sku: String(data.sku || "").trim(),
          name: String(data.name || "").trim(),
          unit: String(data.unit || existing?.unit || "unit").trim() || "unit",
          reorder_point: Math.max(0, Number(data.reorderAt || 0)),
          safety_stock: Math.max(0, Number(data.safetyStock || existing?.safetyStock || 0)),
          lead_time_days: Math.max(0, Math.min(365, Number(data.leadTimeDays || existing?.leadTimeDays || 3))),
          category: String(data.category || "General").trim() || "General",
          cost_cents: Math.max(0, Math.round(Number(data.cost || 0) * 100)),
          sale_price_cents: Math.max(0, Math.round(Number(data.price || 0) * 100)),
          active: data.status !== "Archived"
        }).eq("id", itemId).eq("workspace_id", state.workspace.id).select("id,on_hand").single();
        if (error) throw error;
        currentStock = Number(updated.on_hand || 0);
      } else {
        const { data: created, error } = await client.from("crm_inventory_items").insert({
    ...
    if (type === "employee") {
      ...
      if (!isPlatformWorkspace()) {
        const client = getSupabaseClient();
        if (!client || !authUser) throw new Error("Database authentication is unavailable.");
        const tenantRole = { Admin: "admin", Manager: "manager", Staff: "staff", Viewer: "viewer" }[data.role] || "staff";
        const { data: member, error } = await client.rpc("crm_add_workspace_member", {
          p_workspace: state.workspace.id,
          p_email: String(data.email || "").trim(),
          p_role: tenantRole
        });
        if (error) throw error;
        data.id = member?.profile_id || ui.formDefaults._pickedUserId || data.id;
      } else {
        // Platform staff retain the existing role-management path.
        if (ui.formDefaults._pickedUserId) data.id = ui.formDefaults._pickedUserId;
        if (data.email) {
          ...
          if (matchedContact && matchedContact.id) {
            ...
            if (client) {
              ...
              const { error } = await client.rpc("crm_promote_user", { target_profile: matchedContact.id, new_role: newRole });
              if (error) throw error;
            }
          }
        }
    ...
    if (type === "project") {
      state.tasks = state.tasks.map(task => task.projectId === id ? { ...task, projectId: "", updatedAt: isoNow() } : task);
    }
    clearFormDraft(form);
    ui.modal = null;
    ui.formDefaults = {};
  ...
  async function handleSubmit(form, event) {
    ...
    if (kind === "telegram-chat") {
      ...
      try {
        await socialGatewayRequest("/api/telegram/send", { method: "POST", body: { message } });
        form.reset();
        await loadTelegramOverview(false, true);
        render();
        toast("Sent to Telegram", `Message signed as ${currentUser().name}.`, "success");
      } catch (error) {
        toast("Telegram message not sent", error.message || "The bot could not send this message.", "danger");
      } finally {
  ...
  async function deleteEntity(type, id) {
    ...
    if (type === "employee" && !isPlatformWorkspace()) {
      const sbClient = getSupabaseClient();
      if (!sbClient || !authUser) {
        toast("Team member not removed", "Database authentication is unavailable.", "danger");
        return;
      }
      const { error } = await sbClient.rpc("crm_remove_workspace_member", {
        p_workspace: state.workspace.id,
        p_profile: id
      });
      if (error) {
        toast("Team member not removed", error.message || "The workspace rejected the change.", "danger");
        return;
      }
    }
    if (type === "company" && entity.source === "AkiPasa") {
      const sbClient = getSupabaseClient();
      if (!sbClient || !authUser) {
        toast("Database delete failed", "Database authentication is unavailable.", "danger");
        return;
      }
      const { error } = await sbClient.rpc("operator_delete_catalogue_item", {
        p_target_type: "venue",
        p_target_id: entity.catalogueVenueId || entity.id,
        p_confirmation: "DELETE",
        p_reason: "Deleted from the AkiHQ CRM venue catalogue."
      });
      if (error) {
        toast("Database delete failed", error.message || "The venue could not be removed.", "danger");
        return;
      }
    }
  ...
  async function publishCompanyUnclaimed(id, quiet = false) {
    const company = getEntity("company", id);
    if (!company) return { status: "missing" };
    if (company.catalogueVenueId) return { status: "existing" };
    try {
      const payload = await socialGatewayRequest("/api/crm/leads/publish-unclaimed" + (quiet ? "?mode=batch" : ""), {
        method: "POST",
        body: { workspaceId: state.workspace?.id || "ws_akipasa", company }
      });
      const result = payload?.data || {};
      if (result.skipped) return { status: "skipped", reason: result.reason || "unverified address" };
      company.catalogueVenueId = result.venue_id;
      company.catalogueAddress = result.address;
      company.cataloguePublishedAt = isoNow();
      company.updatedAt = isoNow();
      addAudit("company.unclaimed_venue_published", { companyId: id, venueId: result.venue_id, duplicate: Boolean(result.duplicate) });
      if (!quiet) {
        persist(false);
        await syncCloudWorkspacePushNow();
        render();
      }
      if (!quiet) toast(result.created ? "Venue published" : "Venue linked", company.name + " is now visible as an unclaimed map venue.", "success");
      return { status: result.created ? "created" : "linked" };
    } catch (error) {
      if (!quiet) toast("Venue not published", error.message || "Check the lead address and try again.", "danger");
      return { status: "skipped", reason: error.message || "unverified address" };
    }
  }

  async function publishAllValidCompanies() {
    if (authRole !== "administrator") return;
    const pending = state.companies.filter(company => company.source !== "AkiPasa" && !company.catalogueVenueId);
    if (!pending.length) return toast("Catalogue up to date", "Every company is already linked to a catalogue venue.", "info");
    toast("Publishing verified leads", "Checking " + pending.length + " addresses. This can take a few minutes.", "info");
    const totals = { created: 0, linked: 0, existing: 0, skipped: 0 };
    ...
    const updateProgress = () => {
      const button = document.querySelector('[data-action="publish-all-companies"]');
      if (!button) return;
      button.disabled = true;
      button.innerHTML = icon("map") + " Publishing " + completed + "/" + pending.length;
    };
    updateProgress();
    const workers = Array.from({ length: Math.min(3, pending.length) }, async () => {
      while (cursor < pending.length) {
        const company = pending[cursor++];
        const result = await publishCompanyUnclaimed(company.id, true);
        totals[result.status] = (totals[result.status] || 0) + 1;
        if (result.status === "skipped") {
          const reason = result.reason || "Address could not be uniquely verified";
          skippedReasons.set(reason, (skippedReasons.get(reason) || 0) + 1);
          skippedCompanies.push({ name: company.name || "Unnamed company", address: company.address || "No address", reason });
        } else if (["created", "linked", "existing"].includes(result.status)) {
          publishedCompanies.push({ name: company.name || "Unnamed company", status: result.status });
        }
        completed += 1;
        updateProgress();
      }
    ...
    const runLabel = new Intl.DateTimeFormat("en-GB", {
      dateStyle: "medium",
      timeStyle: "medium",
      timeZone: state.workspace?.timezone || "Europe/Madrid"
    }).format(new Date(finishedAt));
    const groupedFailures = [...skippedReasons.entries()].sort((left, right) => right[1] - left[1]);
    const failureSections = groupedFailures.map(([reason, count]) => {
      const companies = skippedCompanies.filter(item => item.reason === reason);
      return "### " + count + " skipped: " + reason + "\n\n" + companies.map(item => "- **" + item.name + "** — " + item.address).join("\n");
    }).join("\n\n");
    const successfulSection = publishedCompanies.length
      ? publishedCompanies.map(item => "- **" + item.name + "** — " + item.status).join("\n")
      : "No companies were added or linked in this run.";
    const logArticle = {
      id: uid("kb"),
      title: "Catalogue publishing run · " + runLabel,
      category: "Catalogue Publishing Logs",
      authorId: state.currentUserId,
      content: "## Run summary\n\n" +
        "- Started with **" + pending.length + "** unlinked CRM companies\n" +
        "- Processed **" + completed + "**\n" +
        "- Created **" + totals.created + "**\n" +
        "- Linked to existing venues **" + totals.linked + "**\n" +
        "- Already linked **" + totals.existing + "**\n" +
        "- Skipped **" + totals.skipped + "**\n\n" +
        "## Added or linked\n\n" + successfulSection + "\n\n" +
        "## Skipped by reason\n\n" + (failureSections || "No companies were skipped."),
      createdAt: finishedAt,
      updatedAt: finishedAt
    };
    state.knowledge.unshift(logArticle);
    addActivity("article", logArticle.id, "created catalogue publishing log", logArticle.title, "knowledge");
    addAudit("catalogue.publish.completed", { totals, processed: completed, requested: pending.length, knowledgeArticleId: logArticle.id });
    const durableVersion = markDurableRecordDirty();
    persist(false, false);
    let knowledgeSaveError = null;
    let workspaceSaveError = null;
    try {
      await upsertDurableRecord("article", logArticle, durableVersion);
    } catch (error) {
      knowledgeSaveError = error;
    }
    try {
      await syncCloudWorkspacePushNow();
    } catch (error) {
      workspaceSaveError = error;
    }
    render();
    const reasonSummary = groupedFailures
      .slice(0, 3)
      .map(([reason, count]) => count + "× " + reason)
      .join(" · ");
    toast(
      "Catalogue publishing complete",
      completed + "/" + pending.length + " processed · " + (totals.created + totals.linked) + " added or linked · " + totals.skipped + " skipped" + (reasonSummary ? ". " + reasonSummary : ""),
      totals.created || totals.linked ? "success" : "info"
    );
    if (knowledgeSaveError || workspaceSaveError) {
      toast("Publishing log save failed", (knowledgeSaveError || workspaceSaveError).message || "The durable run log could not be saved.", "danger");
    } else {
      toast("Publishing log saved", "Open Knowledge → Catalogue Publishing Logs for the full company-by-company output.", "info");
    }
  }
  ...
  function attachAfterRender() {
    const messages = $("#messages");
    if (messages) messages.scrollTop = messages.scrollHeight;
    if (ui.route === "crm" && ui.crmTab === "ai-team") scrollAIChatToBottom();
  ...
  document.addEventListener("submit", event => {
    ...
    if (!form) return;
    event.preventDefault();
    handleSubmit(form, event).catch(error => {
      console.error("Form submission failed:", error);
      toast("Change not saved", error?.message || "The database rejected the change.", "danger");
    });
  });
    ...
    appRoot.innerHTML = `
      <div class="login-screen">
        <div class="login-card">
          ...
          <h1 style="font-size:18px;font-weight:700;margin:0 0 6px;text-align:center">Sign in to AkiHQ</h1>
          <p style="font-size:12px;color:var(--muted);margin:0 0 20px;text-align:center">For AkiPasa platform staff and active Business Pro workspace members.</p>
          ...
          <p style="font-size:11px;color:var(--muted);text-align:center;margin:20px 0 0">
            Business access and billing are managed via <a href="https://akipasa.com" target="_blank" rel="noopener" style="color:var(--accent-blue)">akipasa.com</a>
          </p>
        </div>
      </div>
    `;
      ...
      <div class="login-screen">
        <div class="login-card" style="max-width:420px;text-align:center">
          ...
          <div style="background:rgba(255,111,133,.08);border:1px solid rgba(255,111,133,.2);border-radius:10px;padding:12px;margin-bottom:20px;font-size:11px;color:var(--danger);line-height:1.4">
            AkiHQ is available to AkiPasa platform staff and active <strong>Business Pro</strong> workspace members only.
          </div>
  ...
  async function loadAccessibleWorkspaces(client, userId, role) {
    ...
    const { data: membershipRows, error } = await client
      ...
      .select("workspace_id,role,status,crm_workspaces!inner(id,name,slug,owner_profile_id,source_venue_id,seat_limit,timezone,currency,status)")
    ...
    const workspaceRows = (membershipRows || []).flatMap(member => {
      ...
      return workspace && workspace.status !== "closed" ? [{
        id: workspace.id, name: workspace.name, slug: workspace.slug, role: member.role,
        ownerId: workspace.owner_profile_id, sourceVenueId: workspace.source_venue_id,
        seatLimit: Number(workspace.seat_limit || 4), timezone: workspace.timezone || "Europe/Madrid",
        currency: workspace.currency || "EUR", toolKeys: []
      }] : [];
    });
    if (["moderator", "administrator"].includes(role)
      && !workspaceRows.some(workspace => workspace.id === "ws_akipasa")) {
  ...
  function activateWorkspace(workspace) {
    ...
    if (!canUseRoute(ui.route)) {
      ...
      try { history.replaceState(null, "", `${location.pathname}${location.search}#/${ui.route}`); } catch {}
    }
    localStorage.setItem(`akihq:selected-workspace:${authUser.id}`, workspace.id);
  ...
  async function switchWorkspace(workspaceId) {
    const client = getSupabaseClient();
    if (client && authUser) {
      const refreshed = await loadAccessibleWorkspaces(client, authUser.id, authRole);
      if (refreshed.length) availableWorkspaces = refreshed;
    }
    const workspace = availableWorkspaces.find(item => item.id === workspaceId);
    if (workspaceSwitchingId || !workspace || workspace.id === state.workspace.id) {
      ui.dropdown = null;
      renderPortal();
      return;
    }
    if (workspaceMutationVersion !== workspaceCommittedVersion) {
      try { await pushWorkspaceSnapshot(workspaceMutationVersion); } catch (error) {
        toast("Workspace not switched", "Save the pending changes before switching workspaces.", "danger");
        return;
      }
    }
    if (window._workspaceSyncSub && client) {
    ...
    try {
      ...
      toast("Workspace opened", "Some live data is still refreshing. You can continue working.", "info");
    } finally {
  ...
  async function bootWithSession(session) {
    ...
    const requestedWorkspaceId = initialSearchParams.get("workspace");
    const savedWorkspaceId = localStorage.getItem(`akihq:selected-workspace:${session.user.id}`);
    const selectedWorkspace = availableWorkspaces.find(workspace => workspace.id === requestedWorkspaceId)
      || availableWorkspaces.find(workspace => workspace.id === savedWorkspaceId)
      || availableWorkspaces.find(workspace => workspace.id === "ws_akipasa")
      || availableWorkspaces[0];
    // Supabase is authoritative; browser memory is only the current workspace render cache.
    ...
    if (existingIndex >= 0) {
      state.employees[existingIndex] = {
        ...
        email: authUser.email,
        name: displayName,
        role: isPlatformWorkspace() ? (role === "administrator" ? "Administrator" : "Moderator / Staff") : capitalize(selectedWorkspace.role),
        status: "Online"
      };
    } else {
      state.employees.push({
        ...
        email: authUser.email,
        role: isPlatformWorkspace() ? (role === "administrator" ? "Administrator" : "Moderator / Staff") : capitalize(selectedWorkspace.role),
        department: isPlatformWorkspace() ? (role === "administrator" ? "Leadership" : "Operations") : "Business",
        status: "Online",
        location: "Spain",
        phone: "",
        joinedAt: isoNow(),
    ...
    if (!window._mailboxSyncInterval) {
      window._mailboxSyncInterval = setInterval(() => loadMailboxOverview(false), 15000);
    }

    if (!window._telegramSyncInterval) {
      window._telegramSyncInterval = setInterval(refreshTelegramInBackground, 5000);
    }
    ...
    if (ui.route === "telegram") loadTelegramOverview(true, true);
    ...
    if (ui.route === "business-workspaces" && isPlatformWorkspace()) loadBusinessWorkspaceOverview(false);
